...
type BreadcrumbConfig = {
    ...
    ColorFun: SheetTree -> string
    ClickAction: SheetTree -> (Msg -> unit) -> unit
    ElementProps: IView list
...
let makeGridFromSheetsWithPositions (cfg: BreadcrumbConfig) (dispatch: Msg -> unit) (posL: (CSSGridPos*SheetTree) list) : IView =
    let btns =
        posL
        |> List.map (fun (pos, sheet) ->
            let crumbId = cfg.BreadcrumbIdPrefix + ":" + sheet.SheetName + ":" + String.concat ":" sheet.LabelPath
            Button.create [
                Button.content $"{sheet.SheetName}"
