...
let drawBlockModelToUserData (model: Model) (userData: UserData) =
    ...
    { userData with
        WireType = bwModel.Type
        ArrowDisplay = bwModel.ArrowDisplay }
...
let spComp (comp: Component) =
    ...
    | Custom { Name = name
               InputLabels = il
               OutputLabels = ol } -> sprintf "Custom:%s(ins=%A:outs=%A)" name il il
    | x -> sprintf "%A" x

let spConn (conn: Connection) = sprintf "Conn:%A" conn.Vertices
...
let updateLdComps (name: string) (changeFun: LoadedComponent -> LoadedComponent) (ldComps: LoadedComponent list) =
    ldComps |> List.map (fun ldc -> if ldc.Name = name then changeFun ldc else ldc)
...
let getCurrSheets (model: Model) =
    match model.CurrentProj with
    | Some proj -> proj.LoadedComponents |> List.map (fun lc -> lc.Name) |> Some
    | None -> None
...
let setWSModel (wsModel: WaveSimModel) (model: Model) =
    ...
    | Some sheets, wsSheet when List.contains wsSheet sheets ->
        { model with
            WaveSim = Map.add wsSheet wsModel model.WaveSim }
    | Some sheets, wsSheet -> failwithf $"What? can't find {wsSheet} in {sheets} to set WSModel"
    | None, _ ->
        printfn
            "\n\n******* What? trying to set wsmod when WaveSimSheet '%A' is not valid, project is closed"
            model.WaveSimSheet
...
let updateWSModel (updateFn: WaveSimModel -> WaveSimModel) (model: Model) =
    ...
    | Some sheets, wsSheet when List.contains wsSheet sheets ->
        ...
        { model with
            WaveSim = Map.add wsSheet (updateFn ws) model.WaveSim }
    | Some sheets, wsSheet -> failwithf $"What? can't find {wsSheet} in {sheets} to set WSModel"
    | None, _ ->
        printfn
            "\n\n******* What? trying to set wsmod when WaveSimSheet '%A' is not valid, project is closed"
...
let setAsyncJobsRunnable dispatch =
    ...
    asyncJobs <- asyncJobs |> List.map (fun job -> { job with ViewHasExecuted = true })
...
let execOneAsyncJobIfPossible (model: Model, cmd: Cmd<Msg>) =
    ...
    |> function
        ...
        | job :: _ ->
            ...
            job.JobWork model |> (fun (model', cmd') -> model', Cmd.batch [ cmd; cmd' ])
