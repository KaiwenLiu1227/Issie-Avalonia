...
module Helpers =
    ...
    module JsonHelpers =
        ...
        type SavedCanvasUnknownWaveInfo<'T> =
            | NewCanvasWithFileWaveSheetInfoAndNewConns of
                JSONCanvasState *
                'T option *
                SheetInfo option *
                System.DateTime
        ...
        let convStateToJC (compL, connL) =
            (List.map convertToJSONComponent compL, connL)

        let stateToJsonString
            (
                cState: CanvasState,
            ...
            //printfn "%A" cState
            try
                JsonSerializer.Serialize<SavedInfo>(
                    NewCanvasWithFileWaveSheetInfoAndNewConns(convStateToJC cState, waveInfo, sheetInfo, time)
                )
        ...
        let stateToJsonStringNew
            ...
            try
                Encode.Auto.toString (
                    space = 0,
                    value = (NewCanvasWithFileWaveSheetInfoAndNewConns(convStateToJC cState, waveInfo, sheetInfo, time)),
                    extra = extraCoder
                )
            with e ->
    ...
    let inline mapPair (f: 'S -> 'T) ((p1, p2): 'S * 'S) = f p1, f p2
    ...
    let inline mapUnion m1 m2 =
        (m2, m1) ||> Map.fold (fun m key value -> Map.add key value m)

    /// create inverse map
    let inline mapInverse (m: Map<'A, 'B>) =
        m |> Map.toSeq |> Seq.map (fun (a, b) -> b, a) |> Map.ofSeq
    ...
    let sprintInitial n (s: string) =
        s |> Seq.truncate n |> Seq.map string |> String.concat ""
    ...
    let tryFindError (lst: Result<'a, 'b> list) : Result<'a list, 'b> =
        ...
        let extractOk el =
            match el with
            | Ok ok -> ok
            | Error _ -> failwith "what? Impossible case in tryFindError"
    ...
    /// Return 2^exponent.
    let pow2 (exponent: int) : int = 1 <<< exponent // TODO use bit-shift.

    /// Return 2^exponent, packed into an int64.
    let pow2int64 (exponent: int) : int64 = 1L <<< exponent
    ...
    let getMemData (address: int64) (memData: Memory1) =
        Map.tryFind address memData.Data |> Option.defaultValue 0L
    ...
    let getNetList ((comps, conns): CanvasState) =
        let id2X f = comps |> List.map f |> Map.ofList
        ...
        let getOutputPortNumber (p: Port) =
            ...
            |> (fun p ->
                ...
                | Some n -> n
                | None -> failwithf "Missing input port number on %A" p.HostId)
            |> OutputPortNumber
        ...
        let updateInputPorts pNum src (comp: NetListComponent) =
            { comp with
                Inputs = Map.add pNum (Some src) comp.Inputs }
        ...
        let updateOutputPorts pNum tgt (comp: NetListComponent) =
            { comp with
                Outputs = Map.add pNum (tgt :: comp.Outputs[pNum]) comp.Outputs }
    ...
    let testMatch (diffX: float) (diffY: float) normRot =
        ...
        let lengthList () : float list =
            ...
            | 90 when ((diffX >= 0) && (diffY >= 0)) ->
                [ s; 0; (diffX - s) / 2.0; (diffY + s); (diffX - s) / 2.0; 0; 0; -s ]
            | 90 when ((diffX >= 0) && (diffY < 0)) -> [ s; 0; (diffX - s); (diffY + s); 0; 0; 0; -s ]
            | 90 when ((diffX < 0) && (diffY >= 0)) -> [ s; 0; 0; (diffY + s); (diffX - s); 0; 0; -s ]
            | 90 when ((diffX < 0) && (diffY < 0)) ->
                [ s; 0; 0; (diffY + s) / 2.0; (diffX - s); (diffY + s) / 2.0; 0; -s ]
...
module PrintSimple =
    ...
    let crop (s: string) = sprintInitial 3 s

    /// print a component simply
    let pComponent (comp: Component) =
        let inPorts = comp.InputPorts |> List.map (fun p -> crop p.Id)
        let outPorts = comp.OutputPorts |> List.map (fun p -> crop p.Id)
    ...
    let pState ((comps, conns): CanvasState) =
        "\n-----COMPS-----\n"
        + (comps |> List.map pComponent |> String.concat "\n")
        + "\n\n-----CONNS----\n"
        + (conns |> List.map pConnection |> String.concat "\n")
        + "\n"
...
module ReduceKeys =
    ...
    let toA36 (n: int) =
        let rec toA36' (alphas: char list) =
            function
    ...
    let toD alphas =
        ...
        alphas |> Seq.toList |> toD' 0

    let getIndexFromReduced (s: string) =
        match s.Length with
        | 0
        | 1 -> failwithf "Can't convert A36 string ids of less than 2 chars to decimal (first char muts be C|W|P)"
        | n ->
            match s[0] with
            | 'C'
            | 'P'
    ...
    type Reducer =
        ...
        member this.ScanConn(conn: Connection) = this.Scan conn.Id
        ...
        member this.ScanProject(p: Project) =
            p.LoadedComponents |> List.iter (fun ldc -> this.ScanCanvas ldc.CanvasState)

        member this.ReduceID (typ: string) (longId: string) =
            match typ with
            | "C"
            | "W"
            | "P" when longId.Length > 10 ->
        ...
        member this.Reduce (typ: string) (longId: string) =
            this.ReduceID typ longId |> Option.defaultValue longId
        ...
        member this.ReducePort(port: Port) =
            Option.defaultValue port (this.ReducePortOpt port)
        ...
        member this.ReduceCanvasState((comps, conns): CanvasState) =
            List.map this.ReduceComp comps, List.map this.ReduceConn conns

        member this.ReduceLDC(ldc: LoadedComponent) =
            { ldc with
                CanvasState = this.ReduceCanvasState ldc.CanvasState }
