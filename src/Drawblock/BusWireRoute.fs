...
let findWireSymbolIntersections (model: Model) (wire: Wire) : BoundingBox list =
    ...
    #else
    let segVertices = List.pairwise wireVertices.[1 .. wireVertices.Length - 1] |> List.zip indexes // do not consider the nubs

    let inputPortId = 
        match wire.InputPort with
        | InputPortId value -> value
    let outputPortId = 
        match wire.OutputPort with
        | OutputPortId value -> value    
        
    let inputCompId = model.Symbol.Ports.[inputPortId].HostId
    let outputCompId = model.Symbol.Ports.[outputPortId].HostId
    #endif
